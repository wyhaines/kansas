...
# Todo: More egregious monkey patching that we can get rid of.
class Object
  def expr_body
    to_s
  end
...
class String
  def expr_body
    Database.sql_escape(self)
  end
end

module Kansas

  class StandardSQLMixin
    ...
    def build_where_from_key(object, key, tableflag = false)
      ...
      fields.each_index do |i|
        ...
        where_ary.push "#{fields[i]} = #{Database.sql_escape(val)}"
      end
    ...
    def build_update(object)
      ...
      object.row.each do |field, val|
        update << " #{field} = #{Database.sql_escape(val)},"
      end
      update.chop!
    end
  end
